...
update_status ModulePhysics::PostUpdate()
{
	...
	if (App->input->GetKey(SDL_SCANCODE_3) == KEY_DOWN)
	{
		...
		b2Vec2 chainPoints[25];
		int j = 0;
		for (int i = 0; i < 50; i++)
		{
			(chainPoints + j)->Set(PIXEL_TO_METERS(*(points + i++)/2), PIXEL_TO_METERS(*(points + i)/2));
			j++;
		}
		...
		chainShape.CreateLoop(chainPoints, 24);
		b2FixtureDef fixture2;
		fixture2.shape = &chainShape;
